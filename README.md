...
└── src/test/java/com/digitalmoneyhouse/testing/
    ├── config/      → TestConfig: URLs, credenciales y datos de prueba fijos
    ├── utils/        → AuthHelper (login/token), DbHelper (seed de datos vía JDBC)
    └── smoke/        → Tests por feature: Auth, User, Account, Card
```

## Casos de prueba manuales

La planilla de casos de prueba (Smoke Test + Regression Test) se encuentra en `Casos_Prueba_Sprint2.xlsx`, con la siguiente convención de IDs:
